...
final contextEngineProvider = Provider<ContextEngine>((ref) => const ContextEngine());
...
class ContextEngine {
  ...
  static final _url = RegExp(r'https?://[^\s]+|\b[\w-]+\.(?:top|xyz|live|info|ru|cn|link)\b',
      caseSensitive: false);
  static final _shortener = RegExp(r'\b(bit\.ly|tinyurl|t\.me|cutt\.ly|rb\.gy|is\.gd)\b',
      caseSensitive: false);
  static final _money = RegExp(r'(₹|rs\.?\s*\d|inr|\$\d|pay|transfer|fee|charge|deposit)',
      caseSensitive: false);
  static final _identity = RegExp(r'\b(otp|kyc|card|cvv|pin|password|aadhaar|pan|verify)\b',
      caseSensitive: false);
  static final _permission = RegExp(
      ...
      caseSensitive: false);
  static final _upi = RegExp(r'\b[\w.-]+@(?:okaxis|oksbi|okhdfcbank|ybl|paytm|upi|ibl)\b',
      caseSensitive: false);
  ...
  ContextExtraction extractLocal(Moment moment) {
    ...
    final risks = <String, double>{
      ...
      'Link / domain': (_shortener.hasMatch(lower) ? 0.8 : 0) +
          (_url.hasMatch(text) ? 0.4 : 0),
      'QR / payment': (_upi.hasMatch(lower) ? 0.8 : 0) +
          (moment.sourceType == 'qr' ? 0.4 : 0),
      'Location': RegExp(r'\b(traffic|delay|eta|cab|airport|commute|boarding)\b',
                  caseSensitive: false)
              .hasMatch(lower)
          ? 0.45
          : 0,
    ...
    final confidence = 0.45 + (risks.values.fold<double>(0, (a, b) => a + b) * 0.06);
