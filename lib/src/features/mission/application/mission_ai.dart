...
class MissionAi {
  ...
  String get _profileBlock {
    ...
    if (p == null || p.displayName.isEmpty) {
      return 'The operator has not filled in a profile yet. Use only the '
          'objective, evidence, and answers provided in this request. Clearly '
          'mark missing profile context instead of inventing personal facts.';
    }
    final parts = <String>[
      'Name: ${p.displayName}',
  ...
  // --- 1. Future-OS orchestration ---------------------------------------------------
  Future<MissionOrchestrationRun> runOrchestration(String objective) async {
    final json = await _json(
      'You are ALTER Mission Control. Run an end-to-end personal AI '
          'orchestration plan against the operator\'s objective using only the '
          'profile and request context provided. Respond with ONLY '
          ...
          '}\n'
          'Provide 4-6 concrete pipeline steps. If a service or data source has no '
          'provided context, mark that step as skipped instead of inventing results.\n\n'
          'OPERATOR PROFILE:\n$_profileBlock',
      'Objective: $objective',
    );
    return MissionOrchestrationRun.fromJson(json);
  }
  ...
  Future<ProofCaptureResult> captureProof({
    ...
    final evidenceText = evidence
        .map(
          (e) =>
              '- [${e.evidenceType}] ${e.title}: ${e.summary} '
              '(source: ${e.source})',
        )
        .join('\n');
    final json = await _json(
